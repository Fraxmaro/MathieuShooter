...
class Game:
    ...
    def start(self):
        ...
        self.spawn_monster(Mummy,side="left")
    ...
    def start_game(self, screen):
        ...
        if self.pressed.get(pygame.K_UP):
            self.player.jump()
        if self.player.rect.y < 500 and not self.pressed.get(pygame.K_UP):
            self.player.jump()

    def check_collision(self, sprite, group):
        return pygame.sprite.spritecollide(sprite, group, False, pygame.sprite.collide_mask)

    def spawn_monster(self, monster_class_name, side="right"):
        self.monsters.add(monster_class_name.__call__(self, side))
